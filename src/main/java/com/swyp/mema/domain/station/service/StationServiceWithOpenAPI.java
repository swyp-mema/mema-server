...
import com.swyp.mema.domain.station.dto.response.subwayInfo.SubwayInfoBasicRes;
...
import com.swyp.mema.domain.station.repository.TmpStationRepository;
...
@Slf4j
@Service
public class StationServiceWithOpenAPI {
	...
	private final TmpStationRepository stationRepository;

	@Value("${api.info.key}")
	private String serviceKey;    // 디코딩된 API 서비스 키

	public StationServiceWithOpenAPI(WebClient.Builder webClientBuilder, StationMasterConverter converter,
									 UserRepository userRepository, MeetRepository meetRepository,
									 MeetMemberRepository meetMemberRepository, TmpStationRepository stationRepository) {
	...
	public TotalStationRes getSubwayInfo() {
	 ...
	 * */
	@Transactional
	public TotalSubwayTimeRes getSubwayTimeByStationId(String stationId, String dailyTypeCode) {
		...
		// OpenAPI 요청 및 JSON 확인
		SubwayTimeBasicRes result = fetchOpenApiForSubwayTime(uri);
		log.info("result : {}", result);

		return converter.toSubwayTimeListResponse(result);
	}

	private SubwayTimeBasicRes fetchOpenApiForSubwayTime(URI uri) {

		// WebClient 요청
		return webClient.get()
			.uri(uri) // 생성한 URI를 그대로 사용
			.retrieve() // 응답 수신
			.bodyToMono(SubwayTimeBasicRes.class)
			.block(); // 동기 방식으로 결과 받기
	}

	private SubwayInfoBasicRes fetchOpenApiForSubwayInfo(URI uri) {

		// WebClient 요청
		return webClient.get()
			.uri(uri) // 생성한 URI를 그대로 사용
			.retrieve() // 응답 수신
			.bodyToMono(SubwayInfoBasicRes.class)
			.block(); // 동기 방식으로 결과 받기
	}
